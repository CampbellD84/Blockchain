...
class Blockchain(object):
    ...
    def new_transaction(self, sender, receiver, amount):
        ...
        # return index of block that will hold transaction
...
def mine():
    ...
    if is_proof_valid:
        ...
        return jsonify({'message': 'Invalid Proof or block changed. Unsuccessful,  Try again.'}), 200
...
def new_transaction():
    ...
    req_params = ['sender', 'receiver', 'amount']

    if not all(k in data for k in req_params):
        response = {'message': "Values are missing."}
        return jsonify(response), 400
